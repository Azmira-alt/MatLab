...
subplot(4,1,2); stem(n_shift, x_shift, 'filled'); title('Shifted x[n-2]'); 
xlim([-4 6]);
subplot(4,1,3); stem(n_rev, x_rev, 'filled'); title('Reversed x[-n]'); 
xlim([-4 6]);
subplot(4,1,4); stem(n_scale, x_scale, 'filled'); title('Scaled x[2n]'); 
xlim([-4 6]); xlabel('n');
